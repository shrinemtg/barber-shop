...
const StyleGallery = () => {
  ...
  const getPosition = (i: number) => {
    ...
    if ((i === current - 1) || (current === 0 && i === images.length - 1)) return -1
    if ((i === current + 1) || (current === images.length - 1 && i === 0)) return 1
    return null
  }
      ...
      <SliderWrap>
        <Cards style={{ width: cardsWidth }}>
          {images.map((img, i) => {
            ...
            // 中央: 0, 左: -1, 右: 1
            let translate = (pos) * (CARD_WIDTH + CARD_GAP)
            return (
              <Card
                key={img.src}
                $center={pos === 0}
                ...
                style={{
                  ...
                  transform: `translateX(${translate}px) scale(${pos === 0 ? SCALE_CENTER : SCALE_SIDE})`,
                  zIndex: pos === 0 ? 2 : 1,
                  filter: pos === 0 ? 'none' : 'grayscale(60%) brightness(0.85)',
                  opacity: 1,
                  pointerEvents: 'auto',
                }}
...
const Card = styled.div<{ $center?: boolean; $side?: boolean }>`
  ...
  /* border-radius: 16px; */
  /* overflow: hidden; */
  box-shadow: ${({ $center }) => $center ? '0 4px 16px rgba(0,0,0,0.12)' : 'none'};
  background: #fff;
  display: flex;
  align-items: center;
  justify-content: center;
  position: absolute;
  top: 0;
  transition: transform 0.4s cubic-bezier(.4,1,.4,1), opacity 0.3s, filter 0.3s;
  cursor: pointer;
  pointer-events: auto;
  z-index: 1;
`
